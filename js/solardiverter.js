...
(function () {
    ...
    var translations = {
        pt: {
            ...
            feat_battery_title: 'Bateria & Prioridade',
            feat_battery_desc: 'Prioridade bateria para inversores híbridos. Suporte a bateria AC independente (Marstek Venus E v1/v2/v3) via Modbus TCP — leitura separada de SOC e potência sem necessidade de inversor híbrido.',
            feat_ac_battery_title: 'Bateria AC Acoplada <span class="sd-pro-only">(PRO)</span>',
            ...
            hw_sensors: 'Sensores',
            hw_sensors_val: 'Até 8x DS18B20 (OneWire)',
            ...
            boards_title: 'Placas Compatíveis (DevKit ESP32)',
            board_u_desc: 'Antena externa IPEX — melhor alcance WiFi. Ideal quando o ESP32 está em caixa metálica.',
            board_d_desc: 'Antena PCB integrada — solução mais compacta para instalação standard.',
            board_oled_desc: 'Com display OLED integrado — mostra estado do sistema, temperaturas e modo de operação em tempo real.',
            boards_warning: 'Atenção na seleção da placa: existem modelos idênticos <strong>sem os pinos PWM</strong> e outros projetados para redes <strong>LoRa</strong> — esses não são compatíveis com este projeto. Confirme sempre o pinout antes de adquirir.',
            step1_title: 'Transferir',
            step1_desc: 'Transfira o ZIP da versão mais recente na secção <a href="#downloads">Transferências</a> e extraia para uma pasta local.',
            step2_title: 'Ligar e Identificar COM',
            step2_desc: 'Ligue o ESP32 via USB. Abra o <strong>Gestor de Dispositivos</strong> → <em>Portas (COM e LPT)</em> e anote o número da porta (ex: COM4). Se não aparecer, instale os <a href="https://www.silabs.com/software-and-tools/usb-to-uart-bridge-vcp-drivers?tab=downloads" target="_blank" rel="noopener">drivers SiLabs CP210x</a>. Use um cabo USB com dados (não apenas de carga).',
            step3_title: 'Flash via flash.bat',
            step3_desc: 'Na pasta extraída, clique na barra de endereços do Explorador de Ficheiros, escreva <code>cmd</code> e prima Enter. Na consola escreva <code>flash.bat COM4</code> (substitua COM4 pelo número anotado) e prima Enter. Se solicitado, pressione o botão <strong>BOOT</strong> do ESP32 alguns segundos para entrar em modo flash e aguarde a conclusão.',
            step4_title: 'Configurar e Ativar',
            step4_desc: 'Ligue-se à rede WiFi <strong>“SolarDiverter-Setup”</strong> (password: <code>12345678</code>) e abra <strong>192.168.4.1</strong>. Configure a rede doméstica, medidor e potência nominal. Insira a chave de ativação recebida por email para desbloquear o firmware.',
            ...
            webui_title: 'Interface Web v1.1.3',
        ...
        en: {
            ...
            feat_battery_desc: 'Battery priority for hybrid inverters. AC-coupled battery support (Marstek Venus E v1/v2/v3) via Modbus TCP — separate SOC and power reading without a hybrid inverter.',
            feat_ac_battery_title: 'AC-Coupled Battery <span class="sd-pro-only">(PRO)</span>',
            ...
            hw_sensors: 'Sensors',
            hw_sensors_val: 'Up to 8x DS18B20 (OneWire)',
            ...
            boards_title: 'Compatible Boards (ESP32 DevKit)',
            board_u_desc: 'External IPEX antenna — better WiFi range. Ideal when the ESP32 is inside a metal enclosure.',
            board_d_desc: 'Integrated PCB antenna — more compact for standard installations.',
            board_oled_desc: 'With integrated OLED display — shows system status, temperatures and operating mode in real time.',
            boards_warning: 'Note on board selection: identical-looking boards exist <strong>without the PWM pins</strong> and others designed for <strong>LoRa</strong> networks — those are not compatible with this project. Always verify the pinout before purchasing.',
            step1_title: 'Download',
            step1_desc: 'Download the latest ZIP from the <a href="#downloads">Downloads</a> section and extract it to a local folder.',
            step2_title: 'Connect & Find COM Port',
            step2_desc: 'Connect the ESP32 via USB. Open <strong>Device Manager</strong> → <em>Ports (COM & LPT)</em> and note the port number (e.g. COM4). If it does not appear, install the <a href="https://www.silabs.com/software-and-tools/usb-to-uart-bridge-vcp-drivers?tab=downloads" target="_blank" rel="noopener">SiLabs CP210x drivers</a>. Use a data-capable USB cable (not charge-only).',
            step3_title: 'Flash via flash.bat',
            step3_desc: 'In the extracted folder, click the address bar in Windows Explorer, type <code>cmd</code> and press Enter. In the console type <code>flash.bat COM4</code> (replace COM4 with your port number) and press Enter. If prompted, hold the <strong>BOOT</strong> button on the ESP32 for a few seconds to enter flash mode and wait for completion.',
            step4_title: 'Configure & Activate',
            step4_desc: 'Connect to the <strong>“SolarDiverter-Setup”</strong> WiFi (password: <code>12345678</code>) and open <strong>192.168.4.1</strong>. Set up your home network, energy meter and nominal power. Enter the activation key received by email to unlock the firmware.',
            ...
            webui_title: 'Web Interface v1.1.3',
        ...
        fr: {
            ...
            boards_title: 'Cartes Compatibles (ESP32 DevKit)',
            board_u_desc: 'Antenne externe IPEX — meilleure portée WiFi. Idéal si l\'ESP32 est dans un boîtier métallique.',
            board_d_desc: 'Antenne PCB intégrée — solution plus compacte pour les installations standard.',
            board_oled_desc: 'Avec écran OLED intégré — affiche l\'état du système, les températures et le mode de fonctionnement en temps réel.',
            boards_warning: 'Attention lors du choix de la carte : des modèles identiques existent <strong>sans les broches PWM</strong> et d\'autres sont conçus pour les réseaux <strong>LoRa</strong> — ceux-ci ne sont pas compatibles. Vérifiez toujours le brochage avant achat.',
            step1_title: 'Télécharger',
            step1_desc: 'Téléchargez le ZIP de la dernière version dans la section <a href="#downloads">Téléchargements</a> et extrayez-le dans un dossier local.',
            step2_title: 'Connecter et trouver le port COM',
            step2_desc: 'Connectez l\'ESP32 via USB. Ouvrez le <strong>Gestionnaire de périphériques</strong> → <em>Ports (COM et LPT)</em> et notez le numéro de port (ex: COM4). Si absent, installez les <a href="https://www.silabs.com/software-and-tools/usb-to-uart-bridge-vcp-drivers?tab=downloads" target="_blank" rel="noopener">drivers SiLabs CP210x</a>. Utilisez un câble USB avec données.',
            step3_title: 'Flash via flash.bat',
            step3_desc: 'Dans le dossier extrait, cliquez sur la barre d\'adresse de l\'Explorateur, tapez <code>cmd</code> et appuyez sur Entrée. Dans la console tapez <code>flash.bat COM4</code> (remplacez COM4 par votre numéro de port) et appuyez sur Entrée. Si demandé, maintenez le bouton <strong>BOOT</strong> de l\'ESP32 quelques secondes et attendez la fin.',
            step4_title: 'Configurer et Activer',
            step4_desc: 'Connectez-vous au WiFi <strong>“SolarDiverter-Setup”</strong> (mot de passe : <code>12345678</code>) et ouvrez <strong>192.168.4.1</strong>. Configurez le réseau, le compteur et la puissance nominale. Entrez la clé d\'activation reçue par email.',
        ...
        es: {
            ...
            boards_title: 'Placas Compatibles (ESP32 DevKit)',
            board_u_desc: 'Antena externa IPEX — mejor alcance WiFi. Ideal cuando el ESP32 está en caja metálica.',
            board_d_desc: 'Antena PCB integrada — solución más compacta para instalación estándar.',
            board_oled_desc: 'Con pantalla OLED integrada — muestra el estado del sistema, temperaturas y modo de operación en tiempo real.',
            boards_warning: 'Atención al seleccionar la placa: existen modelos idénticos <strong>sin los pines PWM</strong> y otros diseñados para redes <strong>LoRa</strong> — no son compatibles. Verifique siempre el pinout antes de adquirir.',
            step1_title: 'Descargar',
            step1_desc: 'Descargue el ZIP de la última versión en la sección <a href="#downloads">Descargas</a> y extráigalo en una carpeta local.',
            step2_title: 'Conectar e Identificar Puerto COM',
            step2_desc: 'Conecte el ESP32 por USB. Abra el <strong>Administrador de Dispositivos</strong> → <em>Puertos (COM y LPT)</em> y anote el número de puerto (ej: COM4). Si no aparece, instale los <a href="https://www.silabs.com/software-and-tools/usb-to-uart-bridge-vcp-drivers?tab=downloads" target="_blank" rel="noopener">drivers SiLabs CP210x</a>. Use un cable USB con datos.',
            step3_title: 'Flash via flash.bat',
            step3_desc: 'En la carpeta extraída, haga clic en la barra de direcciones del Explorador, escriba <code>cmd</code> y pulse Enter. En la consola escriba <code>flash.bat COM4</code> (sustituya COM4 por su puerto) y pulse Enter. Si se le solicita, mantenga pulsado el botón <strong>BOOT</strong> del ESP32 unos segundos y espere a que finalice.',
            step4_title: 'Configurar y Activar',
            step4_desc: 'Conéctese al WiFi <strong>“SolarDiverter-Setup”</strong> (contraseña: <code>12345678</code>) y abra <strong>192.168.4.1</strong>. Configure la red, el medidor y la potencia nominal. Introduzca la clave de activación recibida por email.',
            tip_reset_title: 'Reset de Fábrica:',
            ...
            webui_title: 'Interfaz Web v1.1.3',
